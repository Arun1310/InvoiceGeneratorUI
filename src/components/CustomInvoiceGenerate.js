...
const PdfHighlightViewer = ({ pdfBlob, highlightFields }) => {
...
const CustomInvoiceGenerator = ({ invoiceData}, ref) => {
    ...
    const generateCustomInvoice = () => {
        ...
        const response =  axios.post(`https://localhost:44390/api/Invoice/UploadCustomInvoice/${invoiceData.id}`, formData, {
            headers: {
    ...
    return (
        <div className="grid grid-cols-12 gap-8 mt-12">
          <div className='col-span-11'>
            <PdfHighlightViewer pdfBlob={pdfBlob} highlightFields={highlightFields} />
          </div>
        </div>
    );
};
